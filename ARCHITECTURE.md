...
For *metric shading* (e.g. land price bands), the primary architecture is:

- Boundaries are delivered as **vector tiles**.
- Metrics are delivered via a **geometry-free JSON endpoint**.
- The browser performs a client-side join by stable `area_key` and applies the result via MapLibre **feature-state**.
...
- `admin_areas` (SRID 4326): denormalized table holding admin boundaries across multiple hierarchy depths.
  - Depth is inferred by NULL/NOT NULL checks of `name_1..name_5`.
- `geo.admin_areas` (view): convenience view that exposes computed `area_key` and `depth` alongside geometry.
...
- **Type:** Java 21+, Spring Boot, REST API
- **Purpose:**
...
- **Build:** Multi-stage Dockerfile (Maven build, Temurin JDK 21 runtime)
...
- `GET /api/tiles/{z}/{x}/{y}.mvt`
  ...
  - Clamps depth per tile to the deepest available level *within that tile* (prevents “empty tiles” in countries lacking deep admin levels).
  - Includes `area_key` as a property and uses a stable feature id so MapLibre `feature-state` works reliably.
...
- `GET /api/area-metrics-values` (preferred for UI shading)
  - Returns **metrics only** (no geometry) for a bbox and admin `depth`.
  - Parameters: `minLon`, `minLat`, `maxLon`, `maxLat`, `metricId`, optional `depth`/`zoom`, optional `from`/`to`.
  - Response includes:
    - `items[]` with `{ area_key, avg, band }` used for client-side join.
    - `global_avg` for legend/tooltip context.
  - This avoids duplicating geometry (GeoJSON) that is already provided via vector tiles.

- `GET /api/area-metrics`
  - Returns admin-area **GeoJSON** enriched with the same rollups.
  - Kept primarily for debugging/inspection and simple integrations.
...
**Synthetic / demo data (fast rollups)**

For demos and development, the recommended approach is to seed rollups directly into `facts_agg.area_metric_daily` (fast and deterministic, without point-in-polygon):

- `postgis/dev/seed_dummy_metrics.sh` (wrapper)
- `postgis/dev/seed_dummy_metrics.sql` (SQL)

These scripts populate per-area daily rollups for selected bbox + depth(s), which is sufficient for shading and tooltip/legend behavior.
...
6. **Frontend fetches metric values from `/api/area-metrics-values` for the current bbox + effective depth.**
7. **Frontend joins metrics onto vector-tile features by `area_key` and applies `feature-state` (`band`, `avg`).**
...
### Tile depth clamping (missing deep levels)
Some countries do not have full admin coverage at depths 3–5. To avoid empty vector tiles at high zoom, the tile endpoint clamps the requested depth to the deepest level available *within the tile*.

The UI mirrors this by clamping the metrics request depth to the maximum depth actually present in currently-rendered features (so shading stays aligned with whatever depth the tile endpoint is returning).
