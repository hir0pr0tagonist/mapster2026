-- Synthetic clustered observations generator
--
-- Goal: quickly populate facts.observation + facts_agg.area_metric_daily with data that
-- exhibits spatial clustering (expensive/cheap areas tend to stick together).
--
-- This is intended for demos/dev and does not try to be a statistical model.

CREATE SCHEMA IF NOT EXISTS facts;

-- Gaussian RNG using Box–Muller. VOLATILE because it calls random().
CREATE OR REPLACE FUNCTION facts.randn()
...
-- Generate clustered synthetic observations inside a bbox.
--
-- Inserts into:
-- - facts.metric (if missing)
-- - facts.observation
-- - facts_agg.area_metric_daily (rollups for assigned area and all ancestors)
--
-- Matching:
-- - Assigns each point to the most detailed admin area that covers it.
-- - Option 1 semantics: only aggregate upwards via geo.admin_area_ancestors.
