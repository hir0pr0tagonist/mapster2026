-- Fix synthetic generator (part 2): avoid PL/pgSQL variable ambiguity in INSERT .. RETURNING
--
-- In PL/pgSQL, unqualified identifiers can resolve to variables before columns.
-- The V2 function's RETURNING clause used `metric_id`, which is ambiguous because the
-- function also has a parameter named `metric_id`.
...
BEGIN
  ...
  new_obs AS (
    ...
    RETURNING assigned_area_key, facts.observation.metric_id AS metric_id, (observed_at AT TIME ZONE 'UTC')::date AS day, value
  ),
  rollup AS (
