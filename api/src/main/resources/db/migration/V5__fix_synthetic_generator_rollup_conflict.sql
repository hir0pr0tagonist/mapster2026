-- Fix synthetic generator (part 3): avoid PL/pgSQL variable ambiguity in rollup ON CONFLICT target
--
-- The function parameter named `metric_id` can shadow the column identifier in:
--   ON CONFLICT (area_key, metric_id, day)
--
-- Use the primary key constraint name instead.
...
BEGIN
  ...
  rollup AS (
    ...
    ON CONFLICT ON CONSTRAINT area_metric_daily_pkey DO UPDATE
